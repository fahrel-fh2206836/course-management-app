    ...
    return await Promise.all(
      grouped.map(async (item) => {
        const major = await prisma.major.findUnique({
          where: { majorId: item.majorId },
          select: { majorName: true },
        });
        return { ...item, majorName: major.majorName };
      })
    );
  }
    ...
    return await Promise.all(
      grouped.map(async (item) => {
        const instructor = await prisma.user.findUnique({
          where: { userId: item.instructorId },
  ...
  async getFailRatePerCourse() {
    ...
    return await Promise.all(
      total.map(async (item) => {
        ...
        const fail = fails.find((f) => f.courseId === item.courseId);
        const failCount = fail ? fail._count._all : 0;
        const failRate = `${((failCount / item._count) * 100).toFixed(2)}%`;
  ...
  async getPassRatePerCourse() {
    ...
    return await Promise.all(
      total.map(async (item) => {
        ...
        const pass = passes.find((p) => p.courseId === item.courseId);
        const passCount = pass ? pass._count._all : 0;
        const passRate = `${((passCount / item._count) * 100).toFixed(2)}%`;
