...
export default async function CourseStatusPage() {
  ...
  const ongoingCourse = await appRepo.getCourseByStatus('ongoing');
  const regCourse = await appRepo.getCourseByStatus('registration');
  const notOfferedCourse = await appRepo.getCourseByStatus('notOffered');
