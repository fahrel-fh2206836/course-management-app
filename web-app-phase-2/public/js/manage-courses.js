...
async function onSaveCourseEdit() {
    ...
    // Database update
    await fetch(`${baseUrl}course/${selectedCourse.id}`, {
        method: "PUT",
        headers: {
...
async function renderCourseSection(selectedSections) {
...
async function initCourseSection() {
    renderCourseSection(await (await fetch(`${baseUrl}section?courseId=${selectedCourse.id}`)).json());
}
...
async function handleFilter(e) {
    ...
    query.append("courseId", selectedCourse.id);
    if (semFilter.value !== "All") query.append("semester", semFilter.value);
    if (approvalFilter.value !== "None") query.append("approval", approvalFilter.value);
    if (sectionStatusFilter.value !== "None") query.append("section-status", sectionStatusFilter.value);

    let selectedSections = await (await fetch(`${baseUrl}section?${query.toString()}`)).json();
...
async function onEditSection(sectionId) {
...
async function onCancelSectionEdit(sectionId) {
    setReferences(sectionId);
    const section = await (await fetch(`${baseUrl}section/${sectionId}`)).json();
...
async function onSaveSectionEdit(sectionId) {
    setReferences(sectionId);
    let section = await (await fetch(`${baseUrl}section/${sectionId}`)).json();
    ...
    if(newApprovalStatusDiv !== null) {
        ...
        section.approvalStatus = newApprovalStatus;
    }

    if(newApprovalStatus === "CANCELLED") {
        section.currentSeats = 0;
        await fetch(`/api/registration?sectionId=${sectionId}`, {
            method: 'DELETE'
        });
    }
    ...
    section.sectionStatus = doUpdateSectionStatus === true ? newSectionStatus : section.sectionStatus;
